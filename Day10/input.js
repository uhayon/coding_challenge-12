...
position=<-20655, -20679> velocity=< 2,  2>`.split('\n');

const inputExample = `position=< 9,  1> velocity=< 0,  2>
...
const parseInput = input => {
  let [posText, positionCoordinates, velocityCoordinates] = input.split('=<');
  positionCoordinates = positionCoordinates.replace('> velocity', '');
  velocityCoordinates = velocityCoordinates.replace('>', '');

  let [positionX, positionY] = positionCoordinates.split(',');
  let [velocityX, velocityY] = velocityCoordinates.split(',');
...
module.exports = {
  input: input.map(inp => parseInput(inp)),
